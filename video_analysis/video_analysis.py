...
def get_configuration():
    ...
    config_win.title("视频分析配置")
    config_win.geometry("500x300")
    
    # 变量存储
    video_dir = tk.StringVar(value=os.path.join(os.path.expanduser('~'), 'Desktop'))
    still_dir = tk.StringVar(value=os.path.join(os.path.expanduser('~'), 'Desktop', 'stills'))
    api_key = tk.StringVar(value='')
    
    # 布局
    tk.Label(config_win, text="视频目录:").grid(row=0, column=0, padx=10, pady=10, sticky='e')
    tk.Entry(config_win, textvariable=video_dir, width=40).grid(row=0, column=1)
    tk.Button(config_win, text="浏览...", command=lambda: video_dir.set(filedialog.askdirectory())).grid(row=0, column=2)
    
    tk.Label(config_win, text="定帧图目录:").grid(row=1, column=0, padx=10, pady=10, sticky='e')
    tk.Entry(config_win, textvariable=still_dir, width=40).grid(row=1, column=1)
    tk.Button(config_win, text="浏览...", command=lambda: still_dir.set(filedialog.askdirectory())).grid(row=1, column=2)
    
    tk.Label(config_win, text="Dashscope API Key:").grid(row=2, column=0, padx=10, pady=10, sticky='e')
    tk.Entry(config_win, textvariable=api_key, width=40, show='*').grid(row=2, column=1)
    
    result = {}
    
    def on_confirm():
        nonlocal result
        if not video_dir.get():
            messagebox.showerror("错误", "视频目录不能为空！")
            return
        if not api_key.get():
            if not messagebox.askyesno("警告", "API密钥为空，确定要继续吗？"):
                return
        result = {
            'video_dir': video_dir.get(),
            'still_dir': still_dir.get(),
            'api_key': api_key.get()
        }
        config_win.destroy()
    ...
    tk.Button(config_win, text="确认", command=on_confirm, width=10).grid(row=3, column=1, pady=20, sticky='e', padx=5)
    tk.Button(config_win, text="取消", command=on_cancel, width=10).grid(row=3, column=1, pady=20, sticky='w', padx=5)
    
    config_win.protocol("WM_DELETE_WINDOW", on_cancel)
...
STILL_FRAME_DIR = config['still_dir']
...
def analyze_videos(video_dir, output_csv):
    ...
    root = tk.Tk()
    root.withdraw()
    messagebox.showinfo("分析完成", f"视频分析已完成！\n\n分析镜数: {len(video_files)}\n输出文件: {output_csv}")
